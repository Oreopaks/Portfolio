...
def _score_one(order: dict, profile: dict) -> dict:
    ...
    system = (
        ...
        'Ответ — ТОЛЬКО JSON: {"fit":0-100,"demo":<индекс области или -1>,'
        '"complexity":"низкая|средняя|высокая","why":"<кратко>"}. '
        "fit>=70 — явно его область; 40-69 — смежно; <=20 — не его. "
        "complexity — насколько СЛОЖНО сделать это через ИИ/готовые инструменты: "
        "низкая = типовое на готовом решении (простой бот, шаблон); средняя = умеренный "
        "кастом/интеграции; высокая = много кастома, сложные данные, нетривиальные интеграции. "
...
def score_orders(orders: list[dict], profile: dict, max_items: int = 40) -> list[dict]:
    ...
    for o in orders[:max_items]:
        ...
        comp = str(obj.get("complexity", "средняя")).strip().lower()
        if comp not in ("низкая", "средняя", "высокая"):
            comp = "средняя"
        o = dict(o)
        o["fit"] = fit
        o["demo"] = demo
